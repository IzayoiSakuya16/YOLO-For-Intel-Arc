...
def autocast(enabled: bool, device: str = "xpu"):
    """
...
def cuda_memory_usage(device=None):
    ...
    """
    xpu_info = dict(memory=0)
    if torch.xpu.is_available():
        torch.xpu.empty_cache()
        try:
            yield xpu_info
        finally:
            xpu_info["memory"] = torch.xpu.memory_reserved(device)
    else:
        yield xpu_info
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                mem = 0
                for _ in range(n):
                    with xpu_memory_usage(device) as xpu_info:
                        t[0] = time_sync()
                        y = m(x)
                        t[1] = time_sync()
                        try:
                            ...
                            # print(e)  # for debug
                            t[2] = float("nan")
                    mem += xpu_info["memory"] / 1e9  # (GB)
                    tf += (t[1] - t[0]) * 1000 / n  # ms per op forward
                    tb += (t[2] - t[1]) * 1000 / n  # ms per op backward
                    if max_num_obj:  # simulate training with predictions per image grid (for AutoBatch)
                        with cuda_memory_usage(device) as xpu_info:
                            torch.randn(
                                x.shape[0],
                                max_num_obj,
